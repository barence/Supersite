...
document.getElementById("Submit").onclick=function(){
    ...
    for(let i = 1; i <= NumbChar; i++){
        ...
        if(UserMolecule[i] == Upper[i]){
            if(isNaN(UserMolecule.slice(StartOfSlice,i))==false){
                if(isNaN(UserMolecule.slice(StartOfSlice,i+1))==false){
                    if(isNaN(UserMolecule.slice(StartOfSlice,i+2))==false){
                        Adding*=UserMolecule.slice(StartOfSlice,i+2)-1;
                        i+=2
                    }
                    else{
                        Adding*=UserMolecule.slice(StartOfSlice,i+1)-1;
                        i+=1
                    }
                }
                else{
                    Adding*=UserMolecule.slice(StartOfSlice,i)-1;
                }
            }
